	...
	Context("round trip simple", func() {
		...
		It("non persistent topic", func() {
					...
					Capabilities: []brokerv1beta1.AppCapabilityType{
						{
							// Role: appName, TODO respect role for mtls
							ProducerOf: []brokerv1beta1.AppAddressType{{Name: "APP.JOBS"}},
							ConsumerOf: []brokerv1beta1.AppAddressType{{Name: "APP.JOBS::SUB0"}, {Name: "APP.JOBS::SUB1"}},
						},
					},
			...
			producer := jobTemplate(
				"producer",
				1,
				[]string{"/bin/sh", "-c", "exec java -classpath /opt/amq/lib/*:/opt/amq/lib/extra/* org.apache.activemq.artemis.cli.Artemis producer --protocol=AMQP --user p --password passwd --url " + serviceUrl + " --message-count 1 --destination topic://APP.JOBS;"},
			)
			Expect(k8sClient.Create(ctx, &producer)).Should(Succeed())
			...
			consumer := jobTemplate(
				"consumer",
				1,
				[]string{"/bin/sh", "-c", "exec java -classpath /opt/amq/lib/*:/opt/amq/lib/extra/* org.apache.activemq.artemis.cli.Artemis consumer --protocol=AMQP --user p --password passwd --url " + serviceUrl + " --message-count 1 --destination topic://APP.JOBS::SUB0;"},
			)
			Expect(k8sClient.Create(ctx, &consumer)).Should(Succeed())
