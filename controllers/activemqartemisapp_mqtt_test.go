	...
	Context("mqtt round trip simple", func() {
		...
		It("non persistent", func() {
			...
			Eventually(func(g Gomega) {
				...
				// AMQ229213: User: mqtt-app-test does not have permission='CREATE_NON_DURABLE_QUEUE'
				// for queue my-client.mytopic on address mytopic

				g.Expect(token.Wait()).Should(BeTrue())

				if token.Error() != nil {
					fmt.Printf("Failed to connect to broker: %v\n", token.Error())
				}

				g.Expect(token.Error()).Should(BeNil())
			}, existingClusterTimeout, existingClusterInterval).Should(Succeed())
