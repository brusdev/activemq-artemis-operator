		...
		It("deploy plan 2 clustered", Label("basic-scaledown-check"), func() {
			...
			if os.Getenv("USE_EXISTING_CLUSTER") == "true" {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(pod0AfterScaleDown.Generation).Should(BeEquivalentTo(pod0BeforeScaleDown.Generation))
					g.Expect(pod0AfterScaleDown.ResourceVersion).Should(BeEquivalentTo(pod0BeforeScaleDown.ResourceVersion))
	...
	It("scaledown to the right pod", Label("scale-down-target-selection"), func() {
		...
		if os.Getenv("USE_EXISTING_CLUSTER") == "true" {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(pod1AfterScaleDown.Generation).Should(BeEquivalentTo(pod1BeforeScaleDown.Generation))
				g.Expect(pod1AfterScaleDown.ResourceVersion).Should(BeEquivalentTo(pod1BeforeScaleDown.ResourceVersion))
